...
- **R11 — Contract executability.** `getPublicCampaignDetail` and `getPublicCampaignMediaContent` have stable `operationId`s, required/nullable semantics, valid Problem Details responses, and no `$ref` siblings at touched coordinates. Root `bearerAuth` resolves through `common.yaml`. Split-source validation has zero errors and no new warning fingerprint from this delta; unrelated historical warnings need not be removed.
...
| D2 | Use the exact standalone `PublicCampaignDetail` fields in R3 and nested public schemas described in R4–R8. | Chosen | Resolves schema naming/shape before Build and keeps persistence/internal models non-public by default. |
...
`PublicCampaignDetail` required top-level contract:
...
|---|---|
| `PublicCampaignOrganizerText` | non-empty plain `content`, `source = organizer` |
| `PublicCampaignFundingAvailable` | `availability = available`, `currency = IDR`, decimal-string `target_amount`, decimal-string `collected_amount`, and `progress` |
| `PublicCampaignFundingUnavailable` | `availability = unavailable`, `reason = not_available`; no amount properties |
| `PublicCampaignFundingProgress` | `state = computed \| not_computable`, nullable decimal-string `percentage`, and `relationship = below_target \| target_reached \| above_target \| not_computable` |
| `PublicCampaignMediaAvailable` | `state = available`, `items` with `minItems: 1` |
| `PublicCampaignMediaAbsent` | `state = absent`, `items` with `maxItems: 0` |
| `PublicCampaignMediaUnavailable` | `state = unavailable`, `reason = temporarily_unavailable`, `items` with `maxItems: 0` |
| `PublicCampaignDonationAction` | `availability = unavailable`, `reason = donation_flow_not_available`; no URL/action target |

`PublicCampaignFunding` uses `oneOf` with discriminator `availability`; `PublicCampaignMedia` uses `oneOf` with discriminator `state`. For available funding, `state = not_computable` pairs with `percentage = null` and `relationship = not_computable`; `state = computed` requires a non-negative, uncapped percentage matching `^(0|[1-9][0-9]*)(\.[0-9]{1,2})?$` and one of the other three relationships. All public schema properties are listed in `required`, including deliberately nullable `caption`/`percentage`, so omission does not acquire accidental meaning. OpenAPI 3.0 limitations that cannot express every cross-field condition do not weaken the feature-spec rule or its downstream tests.
...
7. Perform an allowlist/error/cache/generated correspondence sweep across every touched authority. Do not start Build for backend/frontend/topology from this Run.
...
| `api/openapi/campaign.yaml` — public schemas/responses | No explicit public schemas or cache-bearing public error responses exist. | Add exact required schemas/unions and local `PublicCampaignNotFound`/`PublicCampaignUnavailable` responses in §8; remove unreferenced `CampaignDetail`/`OrganizationSummary`; keep historical schemas isolated. |
...
| R3 | Review dereferenced `PublicCampaignDetail` exact property allowlist and generated TS type; confirm no `allOf Campaign/Organization` and no forbidden fields. | Testing | Schema inheritance can silently expose new internal fields. |
...
| R8 | Inspect the discriminated media unions, min/max item constraints, exact item allowlist, and absent vs unavailable distinction. | Testing | Prevents metadata leakage and false no-media presentation. |
...
1. ~~**Exact public schema/property boundary**~~ **RESOLVED — R3–R8 and §8 define the executable names and semantics.**
