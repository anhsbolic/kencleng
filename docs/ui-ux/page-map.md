...
> Boundary: This document does not own exact route implementation, shell layout, visual composition, frontend architecture, or business rules.

This map is derived from current product/domain behavior and exists to keep product surfaces coherent across personas.

Exact route paths may evolve during frontend re-architecture. If a route path conflicts with canonical product/domain requirements, product truth wins.
...
| Surface | UX pattern | Primary purpose |
|---|---|---|
| Organization registration | Form — single submission | Submit organization information/evidence according to the current one-shot registration contract |
...
| Campaign creation/edit | Form — Revisable Submission | Create/revise campaign data where lifecycle permits |
...
Secure-upload reassurance is UX intent where useful and accurate; it does not require a named shared component.
...
- avoid owner-only legal, representative-management, publication, disbursement, or narrative actions when the domain restricts them.
...
### Evidence Journal
A donor-facing post-donation concept for chronological factual updates, reports, provenance, and pending next states.

### Secure/Sensitive Data Presentation
PII, legal documents, tokens, payment/security information, and other sensitive data follow domain/security authority. Design may clarify access but cannot authorize it.
