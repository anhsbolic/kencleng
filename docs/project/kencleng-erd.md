...
-- the hot query: "how many failed attempts for this identifier in the last N minutes"
...
-- anything other than identifier_hash (above) — a BRIN index on the
-- append-only timestamp column is far cheaper to maintain than a
-- second B-tree, useful for occasional "attempts in date range" audit
-- queries without bloating write cost
